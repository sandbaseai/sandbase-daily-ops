...
- Fix: `sandbase-blog` PR #158 updates `LanguageSwitcher.astro` to preserve the current contextual route as a fallback; exact article alternates still take precedence.
...
- Follow-up: verify one English→Chinese and Chinese→English article switch after deployment.
