...
| **source_facts 来源** | 生产 `/v1/models`（410 条 enabled TikHub model）、TikHub 官方文档 docs.tikhub.io、design 文档 REQ-20260728 |
...
| 字段 | 值 |
|---|---|
| **slug** | `tikhub-douyin-data-api-on-sandbase` |
...
| **description** | SandBase now offers 310 Douyin data APIs through the TikHub ecosystem, covering search, user profiles, videos, live rooms, and hot trends. |
...
| 字段 | 值 |
|---|---|
| **slug** | `tikhub-weibo-xiaohongshu-data-on-sandbase` |
| **category** | `product-updates` |
| **type** | `launch` |
| **cover_kind** | `launch` |
...
| **内链目标** | `tikhub-douyin-data-api-on-sandbase`（#2）、`social-media-data-apis-ai-agents-2026`（#1） |
...
| **内链目标** | `tikhub-douyin-data-api-on-sandbase`（#2）、`cron-driven-agents-autonomous-workflows-2026` |
...
| **内链目标** | `tikhub-weibo-xiaohongshu-data-on-sandbase`（#3）、`best-ai-search-apis-agent-workflows-2026` |
...
| **内链目标** | `tikhub-douyin-data-api-on-sandbase`（#2）、`tikhub-weibo-xiaohongshu-data-on-sandbase`（#3）、`multi-channel-ai-agents-slack-discord-whatsapp-2026` |
...
| **description** | Engineering deep-dive: how SandBase turned 431 TikHub operations into a uniform /v1/run contract with naming rules, body wrapping, and fail-closed validation. |
| **内链目标** | `tikhub-douyin-data-api-on-sandbase`（#2）、`mcp-vs-function-calling-ai-agent-tool-integration`、`production-ai-agents-need-a-runtime-layer` |
...
| **角度** | 三种计费模型的真实费用对比：固定/call（TikHub 类）、按秒×分辨率（Kling/H3）、按 token 估算（Gemini）。含 10/100/1000 条视频的预算模板 |
...
| **source_facts 来源** | TikHub 0-0.25 USD/call、LLM 定价数据、design api.md base_price 定义 |
