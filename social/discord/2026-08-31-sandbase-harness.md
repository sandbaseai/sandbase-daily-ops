...
Agent Plugin discovery page: https://agent-plugins.directory/sandbaseai/sandbase-harness — source-indexed, not a security endorsement; its displayed revision is awaiting refresh.

Public community-list result: SandBase Harness is listed under Agent Skills & Tools in https://github.com/aloth/awesome-ai-agents.
...
Hugging Face agent-harness registry: [PR #2432](https://github.com/huggingface/huggingface.js/pull/2432) is now merged at [`56e5168`](https://github.com/huggingface/huggingface.js/commit/56e5168c42132c3b90aacbb1dfdc18f41debde6d). SandBase child shell commands default to `AI_AGENT=sandbase-harness`, while explicit nested-agent markers remain supported. This is attribution integration, not an endorsement or security certification.

New integration reference: [gVisor PR #14517](https://github.com/google/gvisor/pull/14517) documents Docker → `runsc` → gVisor execution for selected DeepSeek Harness commands. SandBase is linked as a complementary Harness-side reference; isolation remains backend/deployment dependent, with no universal kernel-isolation or security-certification claim.
...
Security-oriented discovery: [Awesome-LLMSecOps PR #66](https://github.com/wearetyomsmnv/Awesome-LLMSecOps/pull/66) adds a source-linked SandBase Harness entry under Agentic security. Maintainer review is pending; this is not an endorsement or security certification, and isolation depends on the selected backend and deployment configuration.
...
Existing AI-agent directory review: [Awesome AI Agents PR #467](https://github.com/jim-schwoebel/awesome_ai_agents/pull/467) already contains a single-line SandBase Harness entry. Maintainer review is pending; this follows the existing PR rather than creating a duplicate, and is not an endorsement or security certification.

New active AI-agent directory review: [Jenqyang Awesome AI Agents PR #460](https://github.com/Jenqyang/Awesome-AI-Agents/pull/460), adding SandBase Harness to Applications → Tools. Maintainer review is pending; it follows the destination's OSS and neutral-description rules and is not an endorsement or security certification.
...
Direct follow-up: [E2B Awesome AI Agents PR #1473](https://github.com/e2b-dev/awesome-ai-agents/pull/1473) now carries the factual open-source runtime entry. It is mergeable, but the required CLA check awaits contributor signature; no listing or endorsement is claimed.
...
Link-check follow-up: the failure is the pre-existing `ofekron/better-agent` 404 outside the SandBase diff. The finding is documented; review remains pending.
